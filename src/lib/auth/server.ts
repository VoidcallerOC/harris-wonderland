...
if (isProduction && !databaseUrl) {
  throw new Error(
    "Production database configuration error: DATABASE_URL is required for Better Auth.",
  );
}

// Kick (and share) the development/preview PGLite bootstrap as soon as the auth
// server module loads. This is a no-op for the production PostgreSQL path.
void ensureDbReady();
...
const explicitBaseURL = isProduction
...
// Production/configured apps use PostgreSQL. Development and live preview use
// the embedded PGLite database so Better Auth persists to the same local DB as
// app data. Production can never reach this fallback because src/lib/db.ts
// requires DATABASE_URL before selecting a backend.
