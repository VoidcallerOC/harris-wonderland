...
if (typeof window === "undefined" && isProduction && !databaseUrl) {
  throw new Error(
    "Production database configuration error: DATABASE_URL is required; " +
      "PGlite is only available in development and preview.",
  );
}
...
async function createPgliteSql(): Promise<Sql> {
  ...
  // files are tracked in _migrations. Auth migrations are included only when
  // sign-in is enabled, matching scripts/migrate.mjs. Runs once per module instance
  // — so an HMR reload after adding a migration file applies it live — with
  // passes serialized on a global chain so concurrent callers never
  // double-apply.
  const migrate = async (): Promise<void> => {
    const coreMigrations = import.meta.glob("/migrations/*.sql", {
      query: "?raw",
      import: "default",
      eager: true,
    ...
    const migrations = import.meta.env.VITE_AUTH_ENABLED === "false"
      ? coreMigrations
      : { ...coreMigrations, ...authMigrations };
...
/**
 * Get the shared, **server-only** SQL client. PostgreSQL when `DATABASE_URL` is
 * set, otherwise the local PGLite fallback in development/preview only.
 *
