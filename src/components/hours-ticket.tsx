...
export function HoursTicket({ className }: { className?: string }) {
  ...
  return (
    ...
    >
      ...
      <h2 className="mt-1 font-display text-3xl font-semibold italic leading-none">Shop ticket</h2>
      <div className="mt-4 flex flex-wrap items-center gap-2">
        <Badge
          className={cn("border-ticket-ink/40", status.isOpen ? "text-moss" : "text-ticket-ink/70")}
        >
          <span
            className={cn(
      ...
      <p className="mt-4 border border-dashed border-ticket-ink/30 bg-ticket-ink/10 px-3 py-2 text-sm leading-snug">
        Confirm around holidays. Call ahead before making a special trip.
      </p>
    </aside>
  );
}
