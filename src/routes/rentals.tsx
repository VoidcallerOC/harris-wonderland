...
import { RENTAL_EVENTS } from "@/lib/site";
        ...
        <section className="py-16 sm:py-24">
          <div className="wrap">
            ...
            <Lede className="mt-4 mb-8">
              Every booking is keeper-handled. Species depend on the date, the audience, and where
              it is happening. Ask early for a specific animal.
            </Lede>
            <div className="grid gap-3 md:grid-cols-2 lg:grid-cols-3">
              {RENTAL_EVENTS.map((event) => (
                <article key={event.title} className="border border-border bg-card p-5">
                  ...
                  <h3 className="mt-1 font-display text-card italic text-ticket">{event.title}</h3>
          ...
          <div className="wrap grid items-start gap-10 lg:grid-cols-2">
            <div>
              ...
              <p className="mt-8 max-w-[46ch] text-sm text-muted-foreground">
                Travels from Canton across Connecticut and nearby. Farther out is possible for film
                and production — say so in the details.
              </p>
            </div>
            <RentalForm />
          </div>
